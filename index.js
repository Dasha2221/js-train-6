...
function hasProperty(obj, property) {
  if (property in obj) {
    return `Property ${property} exists.`; // Властивість існує
  } else {
    return `Property ${property} does not exist.`; // Властивість не існує
  }
...
function printKeysAndValues(obj) {
  for (let key in obj) {
    console.log(`Key: ${key}, Value: ${obj[key]}`); // Вивести ключ та відповідне значення
  }
...
let user = {
  ...
  age: 25,
  introduce: function () {
    return `My name is ${this.name} and I am ${this.age} years old.`; // Використовуємо ключове слово this
  },
...
function changeRole(array, newRole) {
  ...
  for (let user of array) {
    // Змінюємо значення поля role на нове значення newRole
    user.role = newRole;
    // Виводимо змінений об'єкт на консоль
    console.log(user);
  }
}
...
function compareObjects(obj1, obj2) {
  // Порівнюємо об'єкти за посиланням
  console.log(obj1 === obj2); // Виводить false

  // Зробимо копію значень об'єкта obj1
  let obj2Copy = { ...obj1 };

  // Порівнюємо скопійований об'єкт та об'єкт obj1
  console.log(obj1 === obj2Copy); // Виводить false
